...
Utilizando el dataset `penguins`del paquete *{palmerpenguins}*, ajuste un modelo lineal para estimar el peso de los pinguinos (en g) `body_mass_g` a partir de la variable continua ancho de pico (en mm) `bill_depth_mm` y la variable discreta especie `species`,sin interacciónm.
...
# library(parameters)
model_parameters(model_dientes)$CI_low[2]
...
CI_in <- tibble(index = 1:200,
                ...
                isin = FALSE)
for (i in 1:200) {
  data_sample <- data %>% sample_n(size = 20)  
...
CI_in_50 <- tibble(index = 1:200,
                ...
                isin = FALSE)
for (i in 1:200) {
  data_sample <- data %>% sample_n(size = 50)  
  model_sample <- lm(Peso ~ Dientes, data_sample)
  CI_in_40$low[i] <- model_parameters(model_sample)$CI_low[2]
  CI_in_40$high[i] <- model_parameters(model_sample)$CI_high[2]
  CI_in_40$isin[i] <- between(1, model_parameters(model_sample)$CI_low[2], model_parameters(model_sample)$CI_high[2])
  print(paste0(i, " low:", CI_in_40$low[i], " high:", CI_in_40$high[i], "IN:", CI_in_40$isin[i]))
}
sum(CI_in_50$isin)/200*100

ggplot() +
  geom_ribbon(data = CI_in, aes(x = index, ymin = low, ymax = high), fill = "#1380A1", alpha = 0.6) + 
  geom_ribbon(data = CI_in_40, aes(x = index, ymin = low, ymax = high), fill = "#ED6A5A", alpha = 0.6) +
...
Utilizando el dataset `birth14` del paquete *{openintro}*, ajuste un modelo lineal para estimar el peso de los recién nacidos (en libras) `weight` utilizando como regresores la longitud del embarazo en semanas `weeks`, la edad de la madre en años `mage`, el sexo del bebé `sex`, si la madre era fumadora `habit`, y el número de visitas al hospital durante el mebarazo `visits`.
